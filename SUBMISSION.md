...
The user pastes any public Ethereum address without signing in, connecting a wallet, or signing a message. The server queries the live Aave V3 Ethereum Subgraph through The Graph Network, rebuilds accrued supply and debt balances from protocol indices and rates, then calculates health factor, severity, Liquidation Buffer, and a fixed `-5%/-10%/-20%` Stress Ladder. Repeated checks produce a Risk Delta only when the source deployment, rules, address, network, and block order are comparable.

Every report includes an Evidence Receipt with the Graph subgraph and deployment, indexed block and timestamp, query time, rules version, scenario assumptions, and position references. An optional OpenAI Responses adapter converts only the precomputed facts into a short explanation. The model cannot alter the report values, and deterministic explanations remain available if the model is absent or fails.
...
The Graph is load-bearing. The live Aave query supplies all balances, reserve indices/rates, liquidation parameters, prices, collateral/eMode state, and source block metadata. There is no mock or alternate production data route; disabling the server-side Graph key stops new analyses. The application rejects stale blocks and indexing errors instead of presenting them as healthy positions.
...
- ETHGlobal track: Continuity / Extend Open Source
- Partner Prize: The Graph — Best AI Tooling or AI Use Case with The Graph (Continuity)
...
5. Expand the Evidence Receipt and point to the deployment, block/time, assumptions, and evidence references.
